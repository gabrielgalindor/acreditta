...
<div class="row">
        <div class="col-lg-12">
        <p class="text-right">
            <a href="{{ route('admin.blog.index') }}" class="btn btn-sm btn-primary"><i class="fa fa-fw fa-list"></i> Ver lista</a>
        </p>
    </div>
</div>
<form role="form" action="{{ route('admin.blog.store') }}" method="POST">
    ...
    <div class="row">
        ...
        <div class="col-lg-3">
            <div class="form-group{{ $errors->has('titulo') ? ' has-error' : '' }}">
                <label>Fecha</label>
                <input type="date" class="form-control datetimepicker" name="fecha" value="{{ old('fecha') }}" required>
                @if ($errors->has('fecha'))
        ...
        <div class="col-lg-6">
            <div class="form-group">
                ...
                <select name="categoria_id" class="form-control">
                    ...
                    <option value="{{ $categoria->id }}" @if($categoria->id==old('categoria_id')) selected @endif>{{ $categoria->categoria_es }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                ...
                <select name="idioma" class="form-control">
                    <option value="es" @if(old('idioma')=='es') selected @endif>EspaÃ±ol</option>
                    <option value="en" @if(old('idioma')=='en') selected @endif>InglÃ©s</option>
                </select>
            </div>
            <div class="form-group">
                <label>Resumen</label>
                <textarea name="resumen" rows="5" class="form-control">{{ old('resumen') }}</textarea>
            </div>
            <div class="form-group">
                ...
                <select name="fuente" class="form-control">
                    ...
                    @foreach($usuarios as $usuario)
                        <option value="{{ $usuario->id }}" @if(old('fuente') === $usuario->id) selected @endif>{{ $usuario->nombre }} - {{ $usuario->email }}</option>
                    @endforeach
                </select>
            </div>
            <!--
            <div class="form-group">
            <label>Fuente / Autor</label>
            <input type="text" class="form-control" name="fuente" value="{{ old('fuente') }}" maxlength="100">
            </div>
            -->
            <!--
            <div class="form-group">
            <label>Enlace</label>
            <input type="text" class="form-control" name="enlace" value="{{ old('enlace') }}" maxlength="200">
            </div>
            -->
        </div>
        <div class="col-lg-6">
            <div class="form-group">
                <label>Cuerpo</label>
                <textarea name="cuerpo" rows="5" class="form-control">{{ old('cuerpo') }}</textarea>
                <p style="padding-top: 10px;"><a href="{{ route('cargafotos') }}" target="_blank" class="btn btn-success">GalerÃ­a de imÃ¡genes</a></p>
            </div>
        </div>
    </div>
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="form-group">
                ...
                <input type="text" name="tags" id="tags" value="{{ old('tags') }}" maxlength="250" style="display: none">
                <div id="mistags"></div>
                <div style="clear:both;"></div>
                <p><input class="form-control" onkeypress="return onKeyPressSoloMin(event);" maxlength="50" name="ntag" id="ntag" value="" placeholder="Buscar / Agregar tag"></p>
